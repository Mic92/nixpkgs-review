...
def pr_flags(
    ...
    pr_parser.add_argument(
        ...
        action="append",
        default=[],
        help="The pull request JSON data returned by the /repos/{owner}/{repo}/pulls/{pull_number} endpoint of the GitHub REST API. Useful if multiple nixpkgs-review instances should operate on the exact same PR commits. Can be specified more than once.",
    )
    pr_parser.add_argument(
        "--no-pr-lookup",
        action="store_true",
        help="Always use the data provided using the --pr-json flag instead of using the GitHub API.",
    )
