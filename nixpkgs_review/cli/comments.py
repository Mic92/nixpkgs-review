...
class Comment:
    ...
    def from_json(data: Dict[str, Any]) -> "Comment":
        return Comment(author=data["author"]["login"],
                       body=data["body"],
...
class ReviewComment(Comment):
    ...
    def from_json(data: Dict[str, Any]) -> "ReviewComment":
        ...
        return ReviewComment(author=data["author"]["login"],
                             body=data["body"],
                             id=data["id"],
...
class Review:
    ...
    def from_json(data: Dict[str, Any], comments: List[ReviewComment]) -> "Review":
        return Review(author=data["author"]["login"],
                      body=data["body"],
                      created_at=parse_time(data["createdAt"]),
                      comments=comments)
...
def get_comments(github_token: str, pr_num: int) -> List[Union[Comment, Review]]:
    ...
    comments: List[Union[Comment, Review]] = [
        Comment.from_json(pr)
    ]
    ...
    for comment in comments:
        if isinstance(comment, Review):
            if comment.body == "" and len(comment.comments) == 0:
                # skip replies
                continue
            print(f"[{comment.created_at}] {bold(comment.author)} reviewed: {comment.body}\n")
            ...
            print(f"[{comment.created_at}] {bold(comment.author)} said: {comment.body}\n")
